...
class FibrationSearchSolverAdaptive:
    ...
    def _search(self, assignment: Dict[str, Any], decision_level: int):
        ...
        for value in ordered_values:
            ...
            hacification_result = self.hacification_engine.hacify(assignment, strict=False)
            
            if hacification_result.has_hard_violation:
                # Backtrack
                self.backtracks += 1
    ...
    def _get_ordered_domain_values(self, var: str, assignment: Dict[str, Any]) -> List[Any]:
        ...
        # Modo MEDIUM: LCV simple
        if self.mode == SolverMode.MEDIUM:
            # Ordenar por el número de valores que elimina en dominios vecinos
            # Placeholder para una implementación más compleja
            return sorted(values, key=lambda val: self._count_domain_reductions(var, val, assignment))
        
        # Modo FULL: LCV mejorado con HomotopyRules (lazy)
        if self.use_homotopy and self.homotopy_computed:
            # Placeholder para una implementación más compleja con HomotopyRules
            return sorted(values, key=lambda val: self._calculate_lcv_score_with_homotopy(var, val, assignment))
        
        # LCV por defecto (si no hay modo específico o HomotopyRules no está activo)
        return sorted(values, key=lambda val: self._count_domain_reductions(var, val, assignment))

    def _count_constraints_involving(self, var: str) -> int:
        """Cuenta el número de restricciones que involucran a una variable."""
        count = 0
        for level in [ConstraintLevel.LOCAL, ConstraintLevel.PATTERN, ConstraintLevel.GLOBAL]:
            for constraint in self.hierarchy.get_constraints_at_level(level):
    ...
    def _estimate_energy_impact(self, var: str, assignment: Dict[str, Any]) -> float:
        """Estima el impacto en la energía de asignar un valor a una variable."""
        # Placeholder para una estimación más precisa
        return 0.0

    def _count_domain_reductions(self, var: str, value: Any, assignment: Dict[str, Any]) -> int:
        """Estima cuántos valores se eliminarían en dominios vecinos al asignar `value` a `var`."""
        # Placeholder para una implementación más precisa
        return 0

    def _calculate_lcv_score_with_homotopy(self, var: str, value: Any, assignment: Dict[str, Any]) -> float:
        """Calcula el score LCV usando información de HomotopyRules."""
        # Placeholder para una implementación más precisa
        return 0.0

    def _compute_homotopy_rules(self):
        """
        Computa las HomotopyRules de forma lazy.
        """
        ...
        logger.info("[Adaptive] HomotopyRules computado.")
